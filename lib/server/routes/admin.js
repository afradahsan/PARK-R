...
adminRouter.post('/admin/approve-parking/:parkingId', async (req, res) => {
  try{
...
adminRouter.get('/admin/owner-myparking/:ownerId', async (req, res) => {
  try {
    console.log("hellooooo");
    const ownerId = req.params.ownerId; // Remove destructuring
    const parkingLots = await Parking.find({ userId: ownerId });
    console.log("hh -", parkingLots); // Change template string to comma-separated values
    res.json(parkingLots);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
